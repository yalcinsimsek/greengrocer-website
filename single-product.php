...
$urid =  substr($urid, 64);  
$urunun_id =  substr($urid, 0, -64);   

$sorgu_urun = mysqli_query($conn,"select * from urun where urid =$urunun_id");
...
if (isset($_POST['submit'])) {
	...
	$miktar = isset($_POST['sayi']) ? $_POST['sayi'] : 1;
if ($miktar < 1) {
    $miktar = 1;
}
    ...
    $urun_ad = $satir_urun['adı'];
    $urun_id = $satir_urun['urid'];

   
    $query = "INSERT INTO sepet (urun_id, ad, miktar) VALUES ('$urun_id', '$urun_ad', '$miktar')";
    ...
    if ($result) {
       ...
       echo '<div style="display: flex; justify-content: center; align-items: center; height: 30vh;">';
        ...
        echo '<h2 style="font-size: 24px; font-weight: bold; margin-bottom: 20px;">Sepet</h2>';
        echo '<p style="color: green;font-size: 18px; font-weight: bold;">Ürün başarıyla sepete eklendi.</p>';
        echo '<a href="shop.php" style="font-size: 18px; font-weight: bold; padding: 8px; background-color: #F28123; color: white; border: none; border-radius: 5px; text-decoration: none; margin-top: 10px;">Ürünlere Dön</a>';
        echo '</div>';
        echo '</div>';
    } else {
        
        echo "Ekleme işlemi başarısız. Hata: " . mysqli_error($conn);
    }
} else {
?>
        ...
        <div class="row">
            <div class="col-md-5">
                <div class="single-product-img">
                    <img src="assets/img/products/<?php echo $satir_urun['resim']?>" alt="">
                </div>
            </div>
            <div class="col-md-7">
                <div class="single-product-content">
                    <h3><?php echo $satir_urun['adı']?></h3>
                    <p class="single-product-pricing"><span>Kg başı</span> <?php echo $satir_urun['fiyat']." "."TL"?></p>
					 <p><strong>Kategori: </strong><?php echo $satir_urun['kategori']?></p>
                    <p><?php echo $satir_urun['acıklama']?></p>
                    <div class="single-product-form">
                        <form action="" method="POST">
                            <input type="number" name="sayi" placeholder="1" min="1">
							<br>
                            <button type="submit" name="submit"  style="font-size: 18px; font-weight: bold; padding: 8px; background-color: #F28123; color: white; border: none; border-radius: 5px; cursor: pointer;"></i> Sepete Ekle</button>
                        </form>
			...
			<div class="row">
				<div class="col-lg-8 offset-lg-2 text-center">
					<div class="section-title">	
						<h3><span class="orange-text">İlgili</span> Ürünler</h3>
						<p>İlginizi çekebilecek bazı ürünler.</p>
					</div>
				</div>
			</div>
			<div class="row">
