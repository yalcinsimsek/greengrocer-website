...
if ($_FILES['resim']['name']) {
    ...
    $resim_path = "assets/img/latest-news/" . $resim;
    
    
    move_uploaded_file($resim_tmp, $resim_path);
    
    
    $sql = "UPDATE haberler SET resim='$resim' WHERE id=$id";
    $conn->query($sql);
}


$sql = "UPDATE haberler SET padı='$padı', baslik='$baslik', haber='$haber', ozet='$ozet', tag='$tag', tag1='$tag1', tag2='$tag2', tag3='$tag3', kaynak='$kaynak' WHERE id=$id";
...
if ($conn->query($sql) === TRUE) {
    echo '<div style="display: flex; justify-content: center; align-items: center; height: 30vh;">';
        ...
        echo '<h2 style="font-size: 24px; font-weight: bold; margin-bottom: 20px;">Haber Güncelleme</h2>';
        echo '<p style="color: green;font-size: 18px; font-weight: bold;">Haber başarıyla güncellendi.</p>';
