						...
						<nav class="main-menu">
							<ul>
								...
								<li><a href="#">Site Bilgileri</a>
									...
									<li   class="current-list-item"><a href="gkutusu.php">Gelen Kutusu</a></li>
...
$sql = "SELECT * FROM iletisim WHERE id LIKE '%$searchTerm%'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
	echo '<div style="text-align: center; margin-bottom: 10px;">'; 
    echo '<form method="GET" action="">'; 
    echo '<input type="text" name="search" placeholder="Mesaj Kodu Girin" value="' . $searchTerm . '" style="margin-bottom: 10px; width: 300px; font-size: 16px;">';
    echo '<br>';
        ...
        <div class="col-lg-4 col-md-6">
            <div class="single-latest-news">
                <div class="news-text-box">
				 <h3><a><?php echo $row['konu']?></a></h3>
                    <p class="blog-meta">
                        <span class="author"><i class="fas fa-user"></i><?php echo $row['ads']?></span>
                        <span class="date"><i class="fas fa-calendar"> Mesaj Kodu:</i><?php echo $row['id']?></span>
                    </p>
                    <p class="excerpt"><?php echo nl2br($row['mesaj'])?></p>
					<p class="excerpt">Mail: <?php echo nl2br($row['mail'])?></p>
					<p class="excerpt">Telefon: <?php echo nl2br($row['tel'])?></p>
                    <input type="button" value="x" style="font-size: 10px; font-weight: bold; padding: 8px; background-color: #F23C3C; color: white; border: none; border-radius: 5px; cursor: pointer; " onclick="confirmDelete(<?php echo $row['id']; ?>)">
                </div>
            </div>
			<br>
			<br>
    ...
    echo '</form>'; 
    echo '</div>';
    echo "Böyle bir kodda mesaj bulunamadı.";
}
?>
...
<script>
    function confirmDelete(id) {
        if (confirm("Gelen mesajı silmek istiyorsunuz. Emin misiniz?")) {
            window.location.href = "msil.php?id="+id;
        }
    }
</script>
