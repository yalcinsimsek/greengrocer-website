...
$sql = "SELECT * FROM haberler WHERE baslik LIKE '%$searchTerm%'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
	echo '<div style="text-align: center; margin-bottom: 10px;">'; 
    echo '<form method="GET" action="">'; 
    echo '<input type="text" name="search" placeholder="Haber Adı Girin" value="' . $searchTerm . '" style="margin-bottom: 10px; width: 300px; font-size: 16px;">';
    echo '<br>';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        <div class="col-lg-4 col-md-6">
            <div class="single-latest-news">
                <a href="single-news.php?hid=<?php echo hash('sha256', rand(1,1000)).$row['id'].hash('sha256', rand(1,1000))?>"><img height='200' width='350' src="assets/img/latest-news/<?php echo $row['resim']?>" alt=""></a>
                <div class="news-text-box">
                    <h3><a href="single-news.php?hid=<?php echo hash('sha256', rand(1,1000)).$row['id'].hash('sha256', rand(1,1000))?>"><?php echo $row['baslik']?></a></h3>
                    <p class="blog-meta">
                        <span class="author"><i class="fas fa-user"></i><?php echo $row['padı']?></span>
                        <span class="date"><i class="fas fa-calendar"></i><?php echo $row['tarih']?></span>
                    </p>
                    <p class="excerpt"><?php echo nl2br($row['ozet'])?></p>
                    <a href="hduzenle.php?hid=<?php echo hash('sha256', rand(1,1000)).$row['id'].hash('sha256', rand(1,1000))?>" class="read-more-btn">Düzenle <i class="fas fa-angle-right"></i></a>
                </div>
            </div>
        </div>
