...
$sql = "SELECT * FROM siparis WHERE sipno LIKE '%$searchTerm%'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
	echo '<div style="text-align: center; margin-bottom: 10px;">'; 
    echo '<form method="GET" action="">'; 
    echo '<input type="text" name="search" placeholder="Sipariş Kodu Girin" value="' . $searchTerm . '" style="margin-bottom: 10px; width: 300px; font-size: 16px;">';
    echo '<br>';
        ...
        <div class="col-lg-4 col-md-6">
            <div class="single-latest-news">
                <div class="news-text-box">
				 <h3><a>SİPARİŞ#<?php echo $row['sipno']?></a></h3>
                    <p class="blog-meta">
                        <span class="author"><i class="fas fa-user"></i><?php echo $row['ad']?></span>
                        <span class="date"><i class="fas fa-calendar"></i><?php echo $row['tarih']?></span>
                    </p>
                    <p class="excerpt">Telefon: <?php echo nl2br($row['tel'])?></p>
					<p class="excerpt">Adres: <?php echo nl2br($row['adres'])?></p>
					<p class="excerpt">Not: <?php echo nl2br($row['ek_not'])?></p>
					<p class="excerpt">Ödeme Yöntemi: <?php echo nl2br($row['oyontem'])?></p>
                    <form action="sipdetay.php" method="get">
    <input type="hidden" name="siparis_id" value="<?php echo $row['sipno']?>">
    <input type="submit" value="Detay" style="font-size: 10px; font-weight: bold; padding: 8px; background-color: #5353ec; color: white; border: none; border-radius: 5px; cursor: pointer;">
	</form>
    ...
    echo '</form>'; 
    echo '</div>';
    echo "Böyle bir kodda sipariş bulunamadı.";
}
?>
