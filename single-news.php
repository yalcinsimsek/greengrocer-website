...
$haberin_id =  substr($habid, 0, -64);   

$sorgu_haber = mysqli_query($conn,"select * from haberler where id =$haberin_id");
$satir_haber = mysqli_fetch_array($sorgu_haber);
?>
<html lang="tr">
<head>
	<meta charset="UTF-8">
				...
				<div class="col-lg-8">
					<div class="single-article-section">
						<div class="single-article-text">
							<div class="single-artcile-bg"><img height='450' width='750' src="assets/img/latest-news/<?php echo $satir_haber['resim']?>" alt=""></div>
							<p class="blog-meta">
							...
							<h2><?php echo nl2br($satir_haber['baslik'])?></h2>
							<p><?php echo nl2br($satir_haber['haber'])?></p>
						</div>
					</div>
				</div>
				...
				<div class="col-lg-4">
					<div class="sidebar-section">
						<div class="recent-posts">
							<h4>Diğer Paylaşımlar</h4>
							<ul>
							<?php 
								$sorgu_haber = mysqli_query($conn,"select * from haberler order by rand() limit 5");
								$say_haber = mysqli_num_rows($sorgu_haber);
								if ( $say_haber > 0 ) {	
								while ( $satir_haber1 = mysqli_fetch_array($sorgu_haber)) {
							?>
								<li><a href="single-news.php?hid=<?php echo hash('sha256', rand(1,1000)).$satir_haber1['id'].hash('sha256', rand(1,1000))?>"><?php echo $satir_haber1['baslik']?></a></li>
							<?php 	
									}
								}
							?>
						...
						<div class="recent-posts">
							...
							<ul>
								...
								<li><a href="<?php echo $satir_haber['kaynak']?>"target="_blank"><?php echo $satir_haber['kaynak']?></a></li>	
							</ul> 
						</div>
						...
						<div class="tag-section">
							...
							<ul>
								...
								if ( $satir_haber['tag'] != "" ){?>
								<li><a ><?php echo $satir_haber['tag']?></a></li>	
								<?php 	
										}
								?>
								<?php 
								...
								if ( $satir_haber['tag1'] != "" ){?>
								<li><a ><?php echo $satir_haber['tag1']?></a></li>	
								<?php 	
										}
								?>
								<?php 
								...
								if ( $satir_haber['tag2'] != "" ){?>
								<li><a ><?php echo $satir_haber['tag2']?></a></li>	
								<?php 	
										}
								?>
								<?php 
								...
								if ( $satir_haber['tag3'] != "" ){?>
								<li><a ><?php echo $satir_haber['tag3']?></a></li>	
								<?php 	
										}
								?>									
							</ul>
